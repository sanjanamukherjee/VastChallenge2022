...
const maxEmployerStats = {
    headCount: 55,
    averageHourlyRate: 42.48572042,
    averageAge: 59,
    averageJoviality: 0.986983183
};
...
const spider = {
    ...
    tooltipOffsetY: -5,
    dotRadius: 5,
    colorAverageEmployer: 'gray',
    colorSelectedEmployer: '#2a4e97',
...
async function drawSpiderChart(selectedEmployerId) {
    ...
    for (const employer of Object.values(dataEmployers)) {
        ...
        for (const jobId of employer.jobs) {
            ...
            for (const participantId of job.participants) {
                const participant = dataParticipants[participantId];
                if (participant === undefined) continue;
                headCount += 1;
                totalHourlyRate += job.hourlyRate;
                totalAge += participant.age;
                totalJoviality += participant.joviality;
            }
        }
    ...
    for (const axis of spider.axisConfig) {
        ...
        if (selectedEmployerId !== -1) {
            selectedEmployerStats[key] = employerStats[selectedEmployerId][key];
        }
    }
    ...
    const buildTooltipHtml = () => {
        if (selectedEmployerId === -1) {
            return `
            <strong>Average Stats of Employers:</strong>
            <br>
            Head Count: ${averageEmployerStats.headCount.toFixed(2)} employees
            <br>
            Average Wage: $${averageEmployerStats.averageHourlyRate.toFixed(2)} per hour
            <br>
            Average Age: ${averageEmployerStats.averageAge.toFixed(2)} years
            <br>
            Average Joviality: ${averageEmployerStats.averageJoviality.toFixed(2)}
        `;
        } else {
            return `
            <strong>Employer ID: ${selectedEmployerId}</strong>
            <br>
            ...
            <br>
            Building ID: ${dataEmployers[selectedEmployerId].buildingId}
        `;
        }
    }
    ...
    spider.chart
        ...
        .style('fill', (d, i) => i === 0 ? spider.colorAverageEmployer : spider.colorSelectedEmployer)
        .style("fill-opacity", 0.1)
        .style('stroke', (d, i) => i === 0 ? spider.colorAverageEmployer : spider.colorSelectedEmployer)
    ...
    spider.chart
        ...
        .join('circle')
        .attr('class', 'myDot')
        .style('fill', (d) => d.index === 0 ? spider.colorAverageEmployer : spider.colorSelectedEmployer)
        .attr('transform', `translate(${spider.chartWidth / 2}, ${spider.chartHeight / 2})`)
...
const Spider = () => {
    ...
    const contextParticipantsData = useMemo(() => d.participantsData, [d]);
    const contextSpiderEmployerId = useMemo(() => d.spiderEmployerId, [d]);
    ...
    useEffect(() => {
        const work = async () => {
            ...
            dataParticipants = contextParticipantsData;

            const selectedEmployerId = contextSpiderEmployerId;

            await drawSpiderChart(selectedEmployerId);

            chartBusy = false;
        }
        work();
    }, [contextJobsData, contextEmployersData, contextParticipantsData, contextSpiderEmployerId]);
